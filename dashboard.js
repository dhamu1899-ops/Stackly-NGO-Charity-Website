...
const panelData={
  programs:{title:'Programs',text:'Explore Stackly NGO charity programs and monitor active community support work across Salem.',cards:[
  ...
  hours:{title:'Volunteer Hours',text:'Track volunteer contribution, monthly service records, completed tasks, and impact ranking.',cards:[
  ...
  events:{title:'Events',text:'View upcoming charity events, completed activities, registration status, and event impact.',cards:[
  ...
  stories:{title:'Stories',text:'Read real impact stories from beneficiaries, donors, volunteers, and community partners.',cards:[
  ...
  certificates:{title:'Certificates',text:'Manage certificates for volunteer service, participation, donation support, and event completion.',cards:[
  ...
  support:{title:'Support',text:'Get help for programs, events, certificates, donations, volunteer tasks, and dashboard access.',cards:[
...
function renderPanel(key){
  ...
  root.innerHTML=`
    ...
    <div class="dash-content-grid attractive-grid">
      ${data.cards.map((c,i)=>`<article class="dash-info attractive-card"><div class="card-count">${String(i+1).padStart(2,'0')}</div><h3>${c[0]}</h3><p>${c[1]}</p><a href="404.html" class="dash-card-link">View details →</a></article>`).join('')}
    </div>
    <div class="dash-bottom-row">
      ...
      <article class="dash-action-box"><h3>Quick Action</h3><p>Use this panel to monitor work progress, check updates, and coordinate NGO support activities.</p><a href="404.html" class="btn">Open Report <span>→</span></a></article>
    </div>`;
}
...
// Logo refresh and safe 404 back reference inside dashboard pages.
document.querySelectorAll('a.logo').forEach((logo)=>{
  logo.addEventListener('click',(e)=>{ e.preventDefault(); window.location.reload(); });
});
document.addEventListener('click',(e)=>{
  const link=e.target.closest('a[href="404.html"]');
  if(link){ localStorage.setItem('stacklyPreviousPage', window.location.pathname.split('/').pop() || 'index.html'); }
});
