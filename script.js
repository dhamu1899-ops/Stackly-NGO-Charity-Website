    ...
    <div class="auth-card clean-auth-card">
      ...
      <div id="loginPanel">
        ...
        <form class="auth-form" id="loginForm" novalidate>
          ...
          <div class="password-wrap">
            ...
            <button type="button" class="eye-toggle" aria-label="Show password"><span class="sr-only">Show password</span></button>
          </div>
          <label class="terms"><input type="checkbox" required><span>I agree to the <a href="404.html">Terms & Conditions</a></span></label>
          <button class="btn full auth-submit" type="submit"><span>Login</span><span aria-hidden="true">-></span></button>
        </form>
        ${googleButton()}
        <div class="auth-actions"><a href="404.html">Forgot password?</a><a href="#" id="showSignup">Create account</a></div>
        <p class="center"><a class="gold-link" href="index.html">Back to Home</a></p>
      ...
      <div id="signupPanel" style="display:none">
        ...
        <form class="auth-form" id="signupForm" novalidate>
          ...
          <label class="terms"><input type="checkbox" required><span>I agree to the <a href="404.html">Terms & Conditions</a></span></label>
          <button class="btn full auth-submit" type="submit"><span>Sign Up</span><span aria-hidden="true">-></span></button>
        </form>
        ${googleButton()}
...
document.addEventListener("click", (event) => {
  ...
  if (event.target.classList.contains("eye-toggle")) {
    const input = event.target.parentElement.querySelector("input");
    input.type = input.type === "password" ? "text" : "password";
    event.target.classList.toggle("is-visible", input.type === "text");
    event.target.setAttribute("aria-label", input.type === "password" ? "Show password" : "Hide password");
    event.target.querySelector(".sr-only").textContent = input.type === "password" ? "Show password" : "Hide password";
  }
});
...
document.addEventListener("submit", (event) => {
  if (event.target.id === "signupForm") {
    ...
    if (!validEmail(email)) {
      showAuthError(form, "Please enter a valid email address with a domain, like name@example.com.");
      return;
    }

    if (password !== confirmPassword) {
      showAuthError(form, "Password and confirm password must match.");
      return;
    }

    showAuthError(form, "Signup successful. Redirecting to login...");
